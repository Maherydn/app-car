...
const CreateMaintenanceForm = ({ maintenance, isOpen, onClose, onSubmit }) => {
  // Etat du formulaire avec des valeurs par défaut
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit(formData); // Soumettre les données (création ou modification)
    ...
    >
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">
          {maintenance?.maintenance_id ? "Edit Maintenance" : "Create Maintenance"}
        </h2>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div>
            <Input
              ...
              required
              color="indigo"
              ref={firstInputRef} // Mettre le focus sur le premier champ
            />
          </div>
          ...
          <div className="flex justify-end gap-4 mt-6">
            ...
            >
              {maintenance?.maintenance_id ? "Save Changes" : "Create Maintenance"}
            </Button>
          </div>
        </form>
      </div>
